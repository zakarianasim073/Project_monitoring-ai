...
export const receiveMaterial = async (req: Request, res: Response) => {
  try {
    ...
    // SECURITY: Use exists for faster validation and avoid hydrating large project arrays
    ...
    console.error(error);
    // SECURITY: Prevent information leakage by using a generic error message
...
export const updatePDRemarks = async (req: Request, res: Response) => {
  try {
    ...
    // SECURITY: Use exists for faster validation
    const projectExists = await Project.exists({ _id: projectId });
    if (!projectExists) return res.status(404).json({ error: 'Project not found' });
