...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // OPTIMIZATION: Use .exists() to validate project without hydrating large sub-document arrays
    ...
    // OPTIMIZATION: Parallelize independent database operations to reduce total latency
    ...
    // 3. OPTIMIZATION: Use bulkWrite with aggregation pipeline for atomic material updates
    // This eliminates N+1 queries and ensures stock doesn't drop below zero in a single roundtrip.
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      const liabilityTask = (async () => {
        ...
        if (subCon) {
          ...
          // Atomic update to project liabilities
    ...
    console.error(error);
    // Sentinel: Use generic error message to prevent Information Leakage
