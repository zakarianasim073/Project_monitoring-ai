...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // 2. Auto-update BOQ executed quantity (if linked) - OPTIMIZATION: Atomic $inc
    ...
    // 3. Auto-deduct material stock - OPTIMIZATION: BulkWrite for performance and atomic clamping
    if (dprData.materialsUsed && dprData.materialsUsed.length > 0) {
      const materialOps = dprData.materialsUsed.map((usage: any) => ({
        updateOne: {
          filter: { _id: usage.materialId },
          // Use aggregation pipeline in update to ensure currentStock doesn't go below 0
          update: [
            {
              $set: {
    ...
    // 5. Add DPR to project - OPTIMIZATION: Consolidated atomic update
