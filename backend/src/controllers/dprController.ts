...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // PERFORMANCE: Use .exists() to avoid hydrating large sub-document arrays in Project model
    ...
    if (dprData.linkedBoqId && dprData.workDoneQty) {
      // PERFORMANCE: Atomic update to prevent race conditions and N+1 overhead
    ...
    if (dprData.materialsUsed && dprData.materialsUsed.length > 0) {
      // PERFORMANCE: Use bulkWrite to eliminate N+1 queries and an aggregation pipeline for atomic clamping
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      // PERFORMANCE: Use .select() and .lean() to minimize memory footprint
    ...
    // PERFORMANCE: Consolidate all project sub-document updates into a single atomic operation
