...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    if (dprData.linkedBoqId && dprData.workDoneQty) {
      // OPTIMIZATION: Atomic update to avoid hydration and race conditions
    ...
    if (dprData.materialsUsed && dprData.materialsUsed.length > 0) {
      // OPTIMIZATION: Use bulkWrite with aggregation pipeline for atomic stock clamping and to eliminate N+1 loop
    ...
    // OPTIMIZATION: Consolidated update eliminates multiple .save() calls and unnecessary hydration
