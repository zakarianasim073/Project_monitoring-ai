...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    if (dprData.linkedBoqId && dprData.workDoneQty) {
      // BOLA: Ensure BOQ Item belongs to the project
    ...
    if (dprData.materialsUsed && dprData.materialsUsed.length > 0) {
      for (const usage of dprData.materialsUsed) {
        // BOLA: Ensure Material belongs to the project
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      // BOLA: Ensure SubContractor belongs to the project
      ...
      if (subCon) {
        ...
        // BOLA & Performance: Push liability ID directly to project
