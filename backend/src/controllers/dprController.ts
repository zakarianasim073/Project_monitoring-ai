...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // SECURITY: Ensures project exists before any sub-resource mutations
    ...
    if (dprData.materialsUsed?.length > 0) {
      // OPTIMIZATION: bulkWrite eliminates N+1 query problem for multi-material updates
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      ...
      if (subCon) {
        ...
        tasks.push(newLiability.save().then(l => {
          projectUpdatePushes.liabilities = l._id;
        }));
      }
    }

    // 5. Parallelize independent database operations
    await Promise.all(tasks);

    // 6. Final atomic update to link DPR and Liabilities to project
    // OPTIMIZATION: Consolidate pushes into a single update to avoid full Project hydration
    await Project.updateOne({ _id: projectId }, { $push: projectUpdatePushes });
