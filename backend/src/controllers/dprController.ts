...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      // Optimized query to fetch only agreedRates
      ...
      if (subCon) {
        ...
        liabilityIds.push((newLiability._id as any).toString());
      }
    }

    // Parallelize core data updates
    await Promise.all(tasks);

    // 5. Atomic Project update to link DPR and Liabilities
