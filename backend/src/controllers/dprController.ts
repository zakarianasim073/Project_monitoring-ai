...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // Parallelize independent operations to minimize latency
    const tasks: Promise<any>[] = [newDPR.save()];

    // 2. Auto-update BOQ executed quantity (if linked)
    if (dprData.linkedBoqId && dprData.workDoneQty) {
      // Atomic update with BOLA protection (scoping to project)
    ...
    if (dprData.materialsUsed && dprData.materialsUsed.length > 0) {
      // Use bulkWrite to eliminate N+1 query problem and minimize roundtrips
    ...
    // Start subcontractor lookup in parallel with other tasks
    ...
    await Promise.all([...tasks, subConPromise]);
    ...
    // 5. Add DPR (and liability) to project in a single atomic operation
    // Using updateOne prevents the need to hydrate and save the entire project document
