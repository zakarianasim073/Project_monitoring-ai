...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // OPTIMIZATION: Use atomic update with $inc to avoid N+1 and race conditions
    ...
    // OPTIMIZATION: Use bulkWrite with an aggregation pipeline to perform atomic stock updates
    // while clamping at zero, eliminating N+1 queries.
    ...
    // 5. Add DPR and any new liabilities to project in a single atomic update
    if (liabilityIds.length > 0) {
      projectUpdates.$push.liabilities = { $each: liabilityIds };
    }
