...
/**
 * Optimized DPR creation with atomic updates and parallel execution.
 * Reduces database roundtrips and avoids hydrating large Project document.
 */
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // 2. Auto-update BOQ executed quantity (if linked) - Atomic $inc to avoid N+1
    ...
    // 3. Auto-deduct material stock - Use bulkWrite for atomic batch update instead of loop with .save()
    ...
    // 5. Update project references - Atomic $push for both DPR and Liability in one call
    const projectUpdate: any = { $push: { dprs: newDPR._id } };
    if (newLiability) {
      projectUpdate.$push.liabilities = newLiability._id;
    }
    tasks.push(Project.updateOne({ _id: projectId }, projectUpdate));

    // OPTIMIZATION: Parallelize independent DB operations
    ...
    // SECURITY: Log original error but return generic message
