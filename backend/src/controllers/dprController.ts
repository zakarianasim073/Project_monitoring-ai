...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // 3. Auto-deduct material stock (Bulk atomic update using aggregation pipeline for safety)
    ...
    // 4. Auto-create subcontractor liability (Parallelized)
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      const handleLiability = async () => {
        ...
        if (subCon) {
          ...
          return Project.updateOne({ _id: projectId }, { $push: { liabilities: newLiability._id } });
        }
      };
      tasks.push(handleLiability());
    }

    // 5. Link DPR to project and execute all tasks in parallel
