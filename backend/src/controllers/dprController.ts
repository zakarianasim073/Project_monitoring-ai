...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // Track updates to apply to project aggregate in a single roundtrip
    const projectUpdates: any = { $push: { dprs: newDPR._id } };

    // 2. Auto-update BOQ executed quantity (if linked) - Atomic increment
    ...
    // 3. Auto-deduct material stock - Batch update to eliminate N+1 overhead
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      // Use projection to minimize memory and enforce BOLA scoping
    ...
    // 5. Finalize project links in one atomic operation
    ...
    console.error(error);
    // Return generic error to avoid leaking implementation details/stack traces
