...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // 2. Auto-update BOQ executed quantity (BOLA protected)
    // OPTIMIZATION: Atomic update avoids fetch-and-save cycle
    ...
    // 3. Auto-deduct material stock (BOLA protected)
    // OPTIMIZATION: bulkWrite reduces database roundtrips to O(1)
    ...
    // 4. Auto-create subcontractor liability (BOLA protected)
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      ...
      if (subCon) {
        ...
        liabilityId = newLiability._id.toString();
      }
    }

    // Wait for all non-dependent tasks to complete
    await Promise.all(tasks);

    // 5. Link DPR and Liability to project atomically
    // OPTIMIZATION: Single updateOne replaces multiple .save() calls on hydrated Project
    ...
    console.error(error);
    // SECURITY: Standardized error message
