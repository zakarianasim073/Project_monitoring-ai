...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // OPTIMIZATION: Use atomic $inc to eliminate hydration and save() cycle
    ...
    // OPTIMIZATION: Use .lean() for faster lookup
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      ...
      if (subCon) {
        ...
        // Collect liability ID for single project update
        projectUpdates.$push.liabilities = newLiability._id;
      }
    }

    // 5. Finalize Project updates in a single roundtrip
