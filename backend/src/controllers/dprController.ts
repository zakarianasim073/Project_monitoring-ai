...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // OPTIMIZATION: Use Project.exists() to avoid hydrating large sub-document arrays (boq, dprs, etc.)
    ...
    // OPTIMIZATION: Use atomic updateOne with $inc to prevent race conditions and skip hydration
    ...
    // OPTIMIZATION: Use bulkWrite to eliminate N+1 query problem and aggregation pipeline to clamp stock at zero
    ...
    // OPTIMIZATION: Use a single updateOne for both IDs to avoid multiple saves and heavy hydration
