...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // We'll collect all parallel operations here
    const tasks: Promise<any>[] = [newDPR.save()];

    // 2. Auto-update BOQ executed quantity (Atomic increment)
    ...
    // 3. Auto-deduct material stock (Bulk atomic update to eliminate N+1)
    ...
    // 4. Auto-create subcontractor liability (Parallel sub-task)
    let newLiabilityId: any = null;
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      const subConTask = (async () => {
      ...
      tasks.push(subConTask);
    }

    // Execute all database updates in parallel
    await Promise.all(tasks);

    // 5. Link new resources to project atomically
