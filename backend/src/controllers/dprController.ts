...
export const createDPR = async (req: Request, res: Response) => {
  try {
    ...
    // OPTIMIZATION: Use .exists() for lightweight validation to avoid hydrating large project arrays
    ...
    // OPTIMIZATION: Use atomic update to eliminate hydration overhead
    ...
    // OPTIMIZATION: Use bulkWrite to eliminate N+1 loop and atomic pipeline to clamp stock at zero
    ...
    if (dprData.subContractorId && dprData.workDoneQty && dprData.linkedBoqId) {
      ...
      if (subCon) {
        ...
        // Queue liability link for single project update
        projectUpdates.$push.liabilities = newLiability._id;
      }
    }

    // 5. Atomic update to project to link DPR and Liability
    // OPTIMIZATION: Use updateOne with $push to avoid hydrating and saving entire Project document
