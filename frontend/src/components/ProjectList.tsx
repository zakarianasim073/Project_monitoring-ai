...
import { Building2, PlusCircle, Lock, Flag, ArrowRight, UserCircle, ChevronDown, Check } from 'lucide-react';
            ...
            {isRoleMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden">
                <div className="px-6 py-3 border-b border-slate-50">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Select Role</span>
                </div>
                {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-6 py-3 hover:bg-slate-50 transition-colors text-slate-700"
                  >
                    <span className={currentUserRole === role ? 'font-bold text-blue-600' : ''}>{role}</span>
