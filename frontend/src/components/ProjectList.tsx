        ...
        <div className="flex items-center gap-4">
          <div className="relative" ref={menuRef}>
            <button
              ...
              className={`flex items-center gap-3 px-5 py-3 bg-white border rounded-2xl transition-all ${isRoleMenuOpen ? 'border-blue-500 ring-4 ring-blue-50' : 'border-slate-200 hover:border-blue-400'}`}
            >
              <UserCircle className="w-6 h-6 text-slate-500" />
              <span className="font-semibold text-slate-700">{currentUserRole}</span>
            </button>

            {/* Role switcher dropdown */}
            {isRoleMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 overflow-hidden py-2 animate-in fade-in zoom-in duration-150">
                <div className="px-5 py-2 mb-1">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Select Role</p>
                </div>
                {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-5 py-3 text-sm font-medium transition-colors ${currentUserRole === role ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    <span>{role}</span>
