...
import { Building2, PlusCircle, Lock, Flag, ArrowRight, UserCircle, Check, ChevronDown } from 'lucide-react';
...
const ProjectList: React.FC<ProjectListProps> = ({ onSwitchRole }) => {
  ...
  const [currentUserRole, setCurrentUserRole] = useState(localStorage.getItem('currentRole') || 'ENGINEER');
        ...
        <div className="flex items-center gap-4">
          <div className="relative" ref={menuRef}>
            <button
              ...
              className={`flex items-center gap-3 px-5 py-3 bg-white border rounded-2xl transition-all ${isRoleMenuOpen ? 'border-blue-500 shadow-md ring-2 ring-blue-50' : 'border-slate-200 hover:border-blue-400'}`}
            >
            ...
            {isRoleMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 py-2 overflow-hidden">
                <div className="px-6 py-2 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Select Role</div>
                {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                  ...
                  >
                    <span className={`font-medium ${currentUserRole === role ? 'text-blue-600' : 'text-slate-700'}`}>{role}</span>
                    {currentUserRole === role && <Check className="w-4 h-4 text-blue-600" />}
                  </button>
                ))}
              </div>
            )}
