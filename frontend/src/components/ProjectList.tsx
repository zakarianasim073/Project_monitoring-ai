        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-3 px-5 py-3 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none transition-all"
            >
              <UserCircle className="w-6 h-6 text-slate-600" />
              <span className="font-medium">{currentUserRole}</span>
              <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isRoleOpen ? 'rotate-180' : ''}`} />
            </button>
            {/* Role switcher dropdown */}
            {isRoleOpen && (
              <div
                role="listbox"
                className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 py-2"
              >
                {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                  <button
                    key={role}
                    role="option"
                    aria-selected={currentUserRole === role}
                    onClick={() => {
                      onSwitchRole(role);
                      setIsRoleOpen(false);
                    }}
                    className="w-full text-left px-6 py-3 hover:bg-slate-50 focus:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 transition-colors"
                  >
                    {role}
                  </button>
                ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((proj: any) => (
          <button
            ...
            className="text-left bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl transition-all cursor-pointer overflow-hidden group focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none"
          >
            <div className="p-8">
