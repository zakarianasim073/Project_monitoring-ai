...
const ProjectList: React.FC<ProjectListProps> = ({ onSwitchRole }) => {
  ...
  const currentUserRole = (localStorage.getItem('currentRole') as UserRole) || 'ENGINEER';
        ...
        <div className="flex items-center gap-4">
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="flex items-center gap-3 px-5 py-3 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 transition-colors"
            >
            ...
            {isRoleMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in duration-200">
                {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-6 py-3.5 hover:bg-slate-50 transition-colors text-sm font-medium"
                  >
                    <span className={currentUserRole === role ? 'text-blue-600' : 'text-slate-700'}>
                      {role}
                    </span>
