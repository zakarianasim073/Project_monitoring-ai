...
import { Building2, ArrowRight, UserCircle, ChevronDown } from 'lucide-react';
              ...
              <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden">
                {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                  <button
                    ...
                    className="w-full text-left px-6 py-4 hover:bg-blue-50 hover:text-blue-600 transition-colors font-medium border-b last:border-0 border-slate-50"
                  >
                    {role}
                  </button>
                ))}
        ...
        {loading ? (
          ...
          projects.map((proj: any) => (
            <button
              ...
              className="bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl transition-all overflow-hidden group text-left focus-visible:ring-2 focus-visible:ring-blue-500 outline-none"
            >
              <div className="p-8">
              ...
              <div className="px-8 py-5 bg-slate-50 border-t flex justify-between items-center group-hover:bg-blue-50 transition-colors">
                ...
                <ArrowRight className="w-5 h-5 text-slate-400 group-hover:text-blue-600 transform group-hover:translate-x-1 transition-transform" />
              </div>
            </button>
          ))
        )}
