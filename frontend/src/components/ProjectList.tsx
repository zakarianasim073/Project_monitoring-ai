        ...
        <div className="flex items-center gap-4">
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-3 px-5 py-3 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 transition-all focus:ring-2 focus:ring-blue-500 outline-none"
            >
            ...
            {isRoleMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="py-2">
                  {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-6 py-3.5 text-sm font-medium transition-colors hover:bg-slate-50 ${
                        currentUserRole === role ? 'text-blue-600 bg-blue-50/50' : 'text-slate-600'
                      }`}
                    >
                      {role}
                      {currentUserRole === role && <Check className="w-4 h-4" />}
                    </button>
                  ))}
                </div>
              </div>
