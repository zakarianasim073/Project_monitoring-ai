        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-3 px-5 py-3 bg-white border border-slate-200 rounded-2xl hover:border-blue-400 transition-colors focus-visible:ring-2 focus-visible:ring-blue-500 outline-none"
            >
              <UserCircle className="w-6 h-6 text-slate-500" />
              <span className="font-medium">{currentUserRole}</span>
              <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${isRoleMenuOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            {isRoleMenuOpen && (
              <>
                ...
                />
                <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden py-2 animate-in fade-in zoom-in duration-200">
                  {(['DIRECTOR', 'MANAGER', 'ENGINEER', 'ACCOUNTANT'] as UserRole[]).map(role => (
                    <button
                      ...
                      className={`w-full text-left px-6 py-3 hover:bg-slate-50 transition-colors ${currentUserRole === role ? 'text-blue-600 bg-blue-50/50 font-medium' : 'text-slate-600'}`}
                    >
                      {role}
                    </button>
                  ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((proj: any) => (
          <button
            ...
            className="bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl transition-all cursor-pointer overflow-hidden group text-left focus-visible:ring-2 focus-visible:ring-blue-500 outline-none"
          >
            ...
            <div className="px-8 py-5 bg-slate-50 border-t flex justify-between items-center group-hover:bg-blue-50 w-full">
